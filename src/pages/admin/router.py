...
from . import auth, dashboard, kanban, crud, contact, profile, htmx, api, importer, invites, users, project
...
# --- Роуты для персонала ---
staff_crud_router.include_router(dashboard.router) 
staff_crud_router.include_router(kanban.router)
staff_crud_router.include_router(profile.router)
...
staff_crud_router.include_router(project.router)

# 2. ТОЛЬКО ДЛЯ АДМИНОВ ОСТАВЛЯЕМ НАСТРОЙКИ
...
admin_only_crud_router.include_router(invites.router)
