...
async def send_new_lead_notification(lead_data: Dict[str, Any]):
    ...
    phone = _escape_markdown(lead_data.get("phone", "N/A"))
    business_type = _escape_markdown(lead_data.get("business_type", "N/A"))
    
    phone_url = f"tel:{''.join(filter(str.isdigit, lead_data.get('phone', '')))}"

    message = (
        f"🔥 *Новый лид из Facebook/Instagram*\n\n"
        f"👤 *Клиент:* {client_name}\n"
        f"📞 *Телефон:* [{phone}]({phone_url})\n"
    ...
    async with httpx.AsyncClient() as client:
        try:
            ...
            log.info(f"Уведомление о лиде '{client_name}' успешно отправлено в Telegram.")
